...
class EmbeddingModel:
    ...
    def get_text_templates(self, text_search):
        ...
        text_inputs = [
            template.format(text_search) for template in text_templates
        ]  # format with class
        return text_inputs
    ...
    def encode_text(self, text_input, apply_templates=True):
        ...
        with torch.no_grad():
            ...
            text_embeddings = (
                self.model.get_text_features(**inputs).detach().cpu().numpy()
            )
    ...
    def encode_images(self, img_paths_list, normalize=True, from_url=False):
        ...
        if not from_url:
            ...
            imgs = [
                Image.open(BytesIO(requests.get(url).content)) for url in img_paths_list
            ]
    ...
    def get_similar_images_indexes(
        self,
        img_embeddings_np,
        input,
        n=5,
        ...
        if input_mode == "text":
            ...
            # Get image embeddings
            input_embedding = self.encode_images(
                input, normalize=True, from_url=from_url
            )
    ...
    def generate_img_embeddings(self, img_paths_list, batch_size, save_path=None):
        ...
        batch_list = [
            img_paths_list[i : i + batch_size]
            for i in range(0, len(img_paths_list), batch_size)
        ]
        batch_embeddings = [
            self.encode_images(batch, normalize=False)
            for batch in tqdm(batch_list, unit="batch")
        ]
        img_embeddings_np = np.concatenate(batch_embeddings, axis=0)

        # Normalize image embeddings
        img_embeddings_np /= np.linalg.norm(
            img_embeddings_np, ord=2, axis=-1, keepdims=True
        )

        # Save image embeddings
        if save_path:
            embed_dict = {
                img_path.name: embedding_np
                for img_path, embedding_np in zip(img_paths_list, img_embeddings_np)
            }
